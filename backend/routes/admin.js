...
router.get('/stats', async (req, res) => {
  try {
    const [regs, leads, adms, enquiries, scholarships, payments, grievances] = await Promise.all([
    ...
    res.json({
      ...
      data: {
        ...
        openTickets: await Ticket.countDocuments({ status: 'New' }),
        recentLogs: recent
      }
    });
  } catch (err) {
...
router.get('/:module', async (req, res) => {
  try {
    ...
    if (search) {
      const searchFields = ['name', 'studentName', 'fullName', 'email', 'mobile', 'whatsapp', 'studentId', 'phone', 'username'];
