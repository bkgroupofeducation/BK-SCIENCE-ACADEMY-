...
const CourseHubPage = ({ courseType, navigateTo }) => {
  ...
  // Auto-scroll logic for Curriculum cards
  useEffect(() => {
  ...
  // Auto-scroll logic for Career cards
  useEffect(() => {
    const container = careerScrollRef.current;
    if (!container) return;
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Cinematic Hero */}
      <section className={`relative min-h-screen flex items-center pt-24 pb-12 lg:pt-0 bg-gradient-to-br ${data.bgGradient} overflow-hidden`}>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-24">
            ...
            <div className="lg:w-1/2">
              <div className="mb-1">
                <div className="inline-block px-4 py-1 bg-brand-red/10 border border-brand-red/20 rounded-full text-brand-red text-[10px] font-black uppercase tracking-[0.3em] mb-2">
                  Academic Blueprint
                </div>
                <h2 className="text-[28px] sm:text-4xl md:text-6xl font-black text-brand-dark uppercase tracking-tighter leading-none whitespace-nowrap">
                  Curriculum <span className="text-brand-red">Structure</span>
                </h2>
              </div>

              <div 
                ref={scrollRef}
                className="flex overflow-x-auto snap-x snap-mandatory pb-8 lg:pb-0 lg:grid gap-6 scrollbar-hide"
              >
                {Object.entries(data.syllabus).map(([subject, topics], idx) => (
                  <div 
                    key={subject} 
                    className="group relative min-w-[85vw] lg:min-w-0 snap-center bg-white p-10 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 hover:border-brand-red/20 transition-all duration-700 hover:-translate-y-2"
                  >
                    ...
                    <h4 className="text-2xl font-black text-brand-dark mb-8 flex items-center gap-4">
                      <span className="w-10 h-10 rounded-2xl bg-brand-red/5 flex items-center justify-center text-brand-red text-lg font-black">
                        0{idx + 1}
                      </span>
                      {subject}
                    </h4>
            ...
            <div className="lg:w-1/2">
              <div className="mb-1">
                <div className="inline-block px-4 py-1 bg-blue-50 border border-blue-100 rounded-full text-blue-600 text-[10px] font-black uppercase tracking-[0.3em] mb-2">
                  Future Landscapes
                </div>
                <h2 className="text-[28px] sm:text-4xl md:text-6xl font-black text-brand-dark uppercase tracking-tighter leading-none whitespace-nowrap">
                  Career <span className="text-brand-red">Horizons</span>
                </h2>
              </div>

              <div 
                ref={careerScrollRef}
                className="relative lg:pl-10 lg:border-l-2 lg:border-dashed lg:border-gray-200 flex lg:flex-col overflow-x-auto lg:overflow-x-visible snap-x snap-mandatory gap-4 lg:space-y-8 pb-8 lg:pb-0 scrollbar-hide"
              >
                {data.careers.map((career, i) => (
                    <div 
                      key={i} 
                      onClick={() => setActiveTopic({ label: career.role, details: career.details, type: 'Career' })}
                      className="group relative min-w-[85vw] lg:min-w-0 snap-center flex items-center justify-between p-6 md:p-8 bg-white rounded-3xl shadow-sm border border-gray-100 hover:bg-brand-dark transition-all duration-500 cursor-pointer"
                    >
                      <div className="absolute -left-[51px] top-1/2 -translate-y-1/2 w-5 h-5 bg-white border-4 border-brand-red rounded-full z-10 group-hover:scale-125 transition-transform hidden lg:block"></div>
              ...
              <div className="mt-2 p-10 md:p-12 bg-brand-red rounded-[3.5rem] text-white shadow-3xl shadow-brand-red/30 relative overflow-hidden group">
                ...
                <p className="font-extrabold text-2xl md:text-3xl leading-tight mb-10 relative z-10">
      ...
      {/* Curriculum Callout */}
      <section className="py-12 bg-brand-dark rounded-[4rem] mx-4 md:mx-10 mb-0 relative overflow-hidden">
      ...
      <CourseExperience courseType={courseType} navigateTo={navigateTo} />

      {/* CTA Final */}
      <section className="py-6 text-center">
        <div className="container mx-auto px-6">
          <h2 className="text-5xl md:text-8xl font-black text-brand-dark tracking-tighter leading-none mb-6">
            Build Your <br />
