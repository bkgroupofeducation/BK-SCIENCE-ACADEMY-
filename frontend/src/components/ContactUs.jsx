...
import { Phone, Mail, MapPin, Globe, ExternalLink, Camera, MessageCircle, Video, Share2, Sparkles, Briefcase } from 'lucide-react';
...
const ContactCard = ({ title, content, subContent, icon: Icon, color = "indigo" }) => (
    <div className="bg-white p-8 rounded-[40px] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.03)] border border-transparent hover:border-brand-red/30 hover:shadow-2xl transition-all duration-500 flex flex-col items-center text-center group hover:-translate-y-2 relative z-10">
        ...
        <a 
            href={title.toLowerCase().includes('call') ? `tel:${content.replace(/\s+/g, '')}` : title.toLowerCase().includes('email') ? `mailto:${content}` : `https://${content}`}
            target={title.toLowerCase().includes('website') ? '_blank' : undefined}
            rel={title.toLowerCase().includes('website') ? 'noopener noreferrer' : undefined}
            className="text-xl font-black text-brand-dark mb-1 tracking-tight hover:text-brand-red transition-colors"
        >
            {content}
        </a>
    ...
    return (
        <div className="min-h-screen bg-white pt-28 md:pt-36 pb-12 px-6 relative">
            ...
            <div className="container mx-auto max-w-7xl relative z-10">
                ...
                {/* Quick Action Cards */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12 animate-fade-up stagger-1">
                        ...
                        {centers.map((center, idx) => (
                            <div key={idx} className="bg-white/80 backdrop-blur-md rounded-[50px] p-10 md:p-14 shadow-2xl shadow-gray-200/50 flex flex-col md:flex-row gap-10 items-start border border-white hover:border-brand-red/20 transition-all duration-700 relative z-10">
                                <div className="flex-1 space-y-8">
                                    ...
                                    <div className="flex flex-wrap gap-8 pt-4">
                                        <div className="flex flex-col">
                                            ...
                                            <a href={`tel:${center.phone.replace(/\s+/g, '')}`} className="font-black text-brand-dark text-lg hover:text-brand-red transition-colors">{center.phone}</a>
                                        </div>
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <a href={`mailto:${center.email}`} className="font-black text-brand-dark text-lg truncate hover:text-brand-red transition-colors">{center.email}</a>
                                        </div>
                                    </div>
                                ...
                                <a 
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="w-full md:w-80 h-80 bg-gray-50 rounded-[40px] overflow-hidden relative group shadow-inner border border-gray-100"
                                >
                ...
                <div className="bg-surface-1 border border-gray-100 rounded-[40px] p-10 md:p-16 text-center relative overflow-hidden shadow-sm">
                    ...
                    <div className="relative z-10">
                        ...
                        <h2 className="text-3xl md:text-4xl font-black text-brand-dark uppercase tracking-tighter mb-4">Stay <span className="text-brand-red">Notified.</span></h2>
                        ...
                        <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                            {[
                                { icon: MessageCircle, label: "Facebook", handle: "@bkgroupofeducation", href: "https://www.facebook.com/profile.php?id=61587514520659" },
                                { icon: Camera, label: "Instagram", handle: "@bkscienceacademy", href: "https://www.instagram.com/bkscienceacademy/" },
                                { icon: Briefcase, label: "LinkedIn", handle: "@bkscienceacademy", href: "https://www.linkedin.com/company/108660164/admin/dashboard/" },
                                { icon: Video, label: "YouTube", handle: "BK Science Academy", href: "https://www.youtube.com/@BKScienceAcademy" }
                            ].map((social, i) => (
                ...
                <div className="mt-20 text-center animate-fade-up">
                    <p className="text-gray-400 font-extrabold text-sm md:text-base max-w-2xl mx-auto leading-relaxed pt-10 border-t border-gray-100">
