...
const SectionDivider = () => (
  <div className="py-2 flex justify-center">
    <div className="w-px h-8 md:h-12 bg-gradient-to-b from-transparent via-brand-red to-transparent opacity-30"></div>
  </div>
);
...
const InformationPage = ({ navigateTo }) => {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-brand-red selection:text-white pb-16">
      ...
      {/* Editorial Hero */}
      <section className="relative min-h-[75vh] flex items-center pt-20 overflow-hidden rounded-b-[4rem] md:rounded-b-[6rem] mx-2 md:mx-6 shadow-2xl">
      ...
      <div className="container mx-auto px-5 md:px-12 mt-6 md:mt-8">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
          ...
          <main className="flex-1 w-full max-w-4xl mx-auto">
            ...
            <motion.section id="about" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="text-[24px] sm:text-3xl md:text-5xl lg:text-6xl whitespace-nowrap font-black text-brand-dark tracking-tighter mb-8 leading-[0.9]">The Indian Institutes of <span className="text-brand-red">Technology</span></h2>
            ...
            <motion.section id="objectives" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }}>
              <div className="flex items-center gap-5 mb-8">
                ...
                <h2 className="text-[24px] sm:text-3xl md:text-4xl lg:text-5xl whitespace-nowrap font-black text-brand-dark tracking-tighter">Primary Core Objectives</h2>
              </div>
              
              <div className="grid md:grid-cols-2 gap-8">
                {[
                  {
                    desc: "To create an environment that encourages freedom of thought & pursuit of excellence, inculcating necessary vision & self-discipline.",
                    bg: "bg-brand-red",
                    text: "text-white",
                    num: "text-white/20"
                  },
                  {
                    desc: "To build a solid foundation of scientific & technical knowledge to prepare competent engineers.",
                    bg: "bg-brand-yellow",
                    text: "text-brand-dark",
                    num: "text-brand-dark/20"
                  },
                  {
                    desc: "To kindle an entrepreneurial spirit among young, talented minds.",
                    bg: "bg-brand-dark",
                    text: "text-white",
                    num: "text-white/10"
                  },
                  {
                    desc: "To prepare students to become outstanding professionals contributing to nation-building.",
                    bg: "bg-orange-500",
                    text: "text-white",
                    num: "text-white/20"
                  }
                ].map((obj, i) => (
                  <div key={i} className={`relative group ${obj.bg} p-8 md:p-10 rounded-[2rem] hover:-translate-y-2 hover:shadow-2xl transition-all duration-500`}>
                    <span className={`absolute top-6 right-8 text-5xl font-black ${obj.num} group-hover:scale-110 transition-transform duration-500`}>0{i+1}</span>
                    <p className={`${obj.text} font-medium text-lg leading-relaxed mt-10 relative z-10`}>{obj.desc}</p>
                  </div>
                ))}
              </div>
            </motion.section>
            ...
            <motion.section id="schedule" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <div className="bg-brand-dark rounded-[3rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl shadow-brand-dark/20">
                ...
                <h2 className="text-[24px] sm:text-3xl md:text-4xl lg:text-5xl whitespace-nowrap font-black mb-4 relative z-10">2026 Examination Schedule</h2>
            ...
            <motion.section id="eligibility" initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="md:pr-12 mb-10 border-b-4 border-brand-dark pb-6">
                <h2 className="text-[28px] sm:text-4xl md:text-5xl lg:text-6xl whitespace-nowrap font-black text-brand-dark tracking-tighter leading-[0.9] mb-6">Eligibility Criteria</h2>
              ...
              <div className="space-y-10">
                {[
                  ...
                  <div key={i} className="flex gap-6 md:gap-12 relative group hover:-translate-y-1 transition-transform duration-500">
                    <div className="w-16 md:w-24 flex-shrink-0 text-right">
                      <span className={`text-4xl md:text-6xl font-black ${crit.color} font-serif drop-shadow-sm`}>{crit.num}</span>
                    </div>
                    <div>
                      <h3 className="text-2xl md:text-3xl font-black text-brand-dark mb-4 group-hover:text-brand-red transition-colors">{crit.title}</h3>
            ...
            <motion.section id="pattern" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6">
                <div className="bg-gray-900 text-white p-3 sm:p-5 md:p-10 rounded-2xl md:rounded-[2rem] flex flex-col shadow-xl">
                  <h3 className="text-[8px] sm:text-[10px] md:text-xs font-black uppercase text-brand-red tracking-widest md:mb-6 mb-2">Execution Mode</h3>
                  <div>
                    <div className="text-xl sm:text-2xl md:text-5xl font-black mb-1 md:mb-4">CBT</div>
                    <p className="text-[8px] sm:text-[10px] md:text-base text-gray-400 font-medium leading-tight md:leading-relaxed">Conducted exclusively in Computer Based Test mode. Practice mock tests extensively on official portals to build familiarity.</p>
                  </div>
                </div>

                <div className="bg-brand-red p-3 sm:p-5 md:p-10 rounded-2xl md:rounded-[2rem] text-white flex flex-col shadow-xl">
                  <h3 className="text-[8px] sm:text-[10px] md:text-xs font-black uppercase text-white/70 tracking-widest md:mb-6 mb-2">Subject Structure</h3>
                  <ul className="space-y-1 md:space-y-4">
                    <li className="flex flex-col xl:flex-row xl:justify-between xl:items-center border-b border-white/20 pb-1 md:pb-2">
                      <span className="font-bold text-[9px] sm:text-[11px] md:text-xl leading-tight">Physics</span>
                      <span className="font-serif font-medium opacity-80 text-[7px] sm:text-[9px] md:text-base whitespace-nowrap">Both Papers</span>
                    </li>
                    <li className="flex flex-col xl:flex-row xl:justify-between xl:items-center border-b border-white/20 pb-1 md:pb-2">
                      <span className="font-bold text-[9px] sm:text-[11px] md:text-xl leading-tight">Chemistry</span>
                      <span className="font-serif font-medium opacity-80 text-[7px] sm:text-[9px] md:text-base whitespace-nowrap">Both Papers</span>
                    </li>
                    <li className="flex flex-col xl:flex-row xl:justify-between xl:items-center pb-1 md:pb-2">
                      <span className="font-bold text-[9px] sm:text-[11px] md:text-xl leading-tight">Maths</span>
                      <span className="font-serif font-medium opacity-80 text-[7px] sm:text-[9px] md:text-base whitespace-nowrap">Both Papers</span>
                    </li>
                  </ul>
                </div>

                <div className="bg-white border md:border-2 border-gray-100 p-3 sm:p-5 md:p-10 rounded-2xl md:rounded-[2rem] flex flex-col shadow-xl hover:border-gray-300 transition-colors">
                  <h3 className="text-[8px] sm:text-[10px] md:text-xs font-black uppercase text-gray-400 tracking-widest md:mb-6 mb-2">Question Archetypes</h3>
                  <div className="space-y-1 md:space-y-4">
                    <p className="text-brand-dark font-black text-[9px] sm:text-[11px] md:text-lg leading-tight">Single & Multi Choice MCQs</p>
                    <p className="text-brand-dark font-black text-[9px] sm:text-[11px] md:text-lg leading-tight">Numerical Value-Based</p>
                    <p className="text-brand-dark font-black text-[9px] sm:text-[11px] md:text-lg leading-tight">Matching / Matrix Types</p>
                    <div className="pt-1 md:pt-4 border-t border-gray-100">
                      <p className="text-[7px] sm:text-[9px] md:text-xs font-bold text-brand-red uppercase tracking-widest leading-tight">+3 Correct / Negative</p>
                    </div>
                  </div>
                </div>
              </div>
