...
const CounterStat = ({ value, label, icon, delay }) => {
  ...
  return (
    <div
      ref={ref}
      className={`relative group bg-white/80 backdrop-blur-md border border-white/40 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl shadow-gray-200/40 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="absolute -top-2 -right-2 md:-top-4 md:-right-4 w-8 h-8 md:w-12 md:h-12 bg-brand-red rounded-xl md:rounded-2xl flex items-center justify-center text-white text-sm md:text-xl shadow-lg shadow-brand-red/30 group-hover:scale-110 transition-transform duration-500">
        {icon}
      </div>
      <div className="text-xl md:text-4xl lg:text-5xl font-black text-brand-dark mb-1 tabular-nums tracking-tighter">
        {value}
      </div>
      <div className="text-[8px] md:text-xs font-black text-gray-400 uppercase tracking-[0.2em] md:tracking-[0.25em]">{label}</div>
    </div>
  );
};
...
const CoursePage = ({ config, navigateTo }) => {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-brand-red selection:text-white pb-20">
      {/* Hero Section */}
      <section className="relative min-h-[70vh] flex items-center bg-brand-dark overflow-hidden pt-32 pb-16 lg:pt-40 lg:pb-24">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div className={`transition-all duration-1000 ${heroVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
              ...
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-white leading-[0.9] tracking-tighter mb-8">
                {config.heroTitle}
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-brand-yellow to-orange-400">
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                >
                  ...
                  <span className="relative z-10 text-sm uppercase tracking-widest">Inquiry Now</span>
                </button>
                
                <div className="flex -space-x-3">
                  {['D', 'A', 'R', 'S'].map((initial, i) => (
      ...
      {/* Stats Section */}
      <section className="py-4 md:py-6 relative bg-gray-50/50">
      ...
      {/* Syllabus Section */}
      <section className="py-6 md:py-8 bg-brand-dark relative overflow-hidden rounded-[3rem] mx-4 md:mx-8">
        ...
        <div className="container mx-auto px-5 md:px-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            {config.whyReasons.map((item, i) => {
              const themes = [
                { bg: "bg-indigo-50", text: "text-indigo-600", border: "group-hover:border-indigo-200", icon: "bg-indigo-600", hover: "group-hover:bg-indigo-600" },
                { bg: "bg-emerald-50", text: "text-emerald-600", border: "group-hover:border-emerald-200", icon: "bg-emerald-600", hover: "group-hover:bg-emerald-600" },
                { bg: "bg-amber-50", text: "text-amber-600", border: "group-hover:border-amber-200", icon: "bg-amber-600", hover: "group-hover:bg-amber-600" },
                { bg: "bg-rose-50", text: "text-rose-600", border: "group-hover:border-rose-200", icon: "bg-rose-600", hover: "group-hover:bg-rose-600" }
              ];
              const theme = themes[i % themes.length];
              
              return (
                <div key={i} className={`group relative ${theme.bg} border border-transparent ${theme.border} rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl text-center`}>
                  <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl ${theme.icon} flex items-center justify-center text-white mx-auto mb-6 md:mb-8 shadow-lg shadow-black/5`}>
                    {item.icon}
                  </div>
                  <h3 className={`text-sm md:text-xl font-black text-brand-dark mb-2 md:mb-4 tracking-tight uppercase`}>{item.title}</h3>
                  <p className="text-[10px] md:text-base text-gray-500 leading-relaxed font-medium">{item.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Premium Course Facilities & Experience Section */}
      <CourseExperience courseType={config.shortName || 'jee'} navigateTo={navigateTo} />

      {/* FAQ Section */}
      <section className="py-6 md:py-8 bg-gray-50 rounded-t-[3rem]">
        <div className="container mx-auto px-5 md:px-10">
      ...
      {/* Call to Action */}
      <section className="relative py-8 md:py-12 bg-brand-dark overflow-hidden rounded-b-[3rem] mx-4 md:mx-8 mb-8 shadow-2xl">
      ...
      <div className="grid lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 p-10 md:p-12 lg:p-16 flex flex-col justify-between">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12">
            ...
            <div>
              <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Demo</div>
